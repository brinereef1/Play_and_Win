...
public class BetHistoryManager : MonoBehaviour
{
    ...
    private string bet_history_api_url = "http://13.234.117.221:2556/api/v1/user/userbethistory_dice";
    ...
    IEnumerator BetHistoryRequest()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(bet_history_api_url))
        {
            ...
            Debug.Log(response);
            LuckySevenBetResponse betResponse = JsonConvert.DeserializeObject<LuckySevenBetResponse>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                foreach (var item in betResponse.data)
                {
                    ...
                    var Script = win.transform.GetComponent<BetHistoryDisplay>();

                    // Set the values including the formatted IST date
                    Script.SetWinData(item.betAmount, item.gameRoundIdgenerated,item.categoryName);
                }
            }
...
[System.Serializable]
public class LuckySevenBetDatum
{
...
[System.Serializable]
public class LuckySevenBetResponse
{
    public bool success { get; set; }
    public List<LuckySevenBetDatum> data { get; set; }
}