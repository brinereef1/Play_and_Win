...
public class SRLastTenWinnersHistoryManager : MonoBehaviour
{
    ...
    IEnumerator LastTenWinHistoryRequest()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(api_url))
        {
            ...
            Debug.Log("Response from last10: " + response);

            SRLastTenWinResponse lastTen_winResponse = JsonConvert.DeserializeObject<SRLastTenWinResponse>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                foreach (var item in lastTen_winResponse.games)
                {
                    ...
                    var Script = win.transform.GetComponent<SRLastTenWinnersHistoryDisplay>();

                    Script.SetLastTenWinData(item.gameRoundId, item.chosen.Keys.First().ToString());
                }
...
[System.Serializable]
public class SRLastTenDatum
{
    public string gameRoundId { get; set; }
    public Dictionary<string, double> chosen = new Dictionary<string, double>();
}

[System.Serializable]
public class SRLastTenWinResponse
{
    public List<SRLastTenDatum> games { get; set; }
}