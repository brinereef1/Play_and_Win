...
public class SuperRouletteBetHistoryManager : MonoBehaviour
{
    ...
    private string bet_history_api_url = "http://13.234.117.221:2556/api/v1/user/userbethistory_roulette";
    ...
    IEnumerator BetHistoryRequest()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(bet_history_api_url))
        {
            ...
            SPBetResponse betResponse = JsonConvert.DeserializeObject<SPBetResponse>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                foreach (var item in betResponse.betHistory)
                {
                    GameObject bet = Instantiate(betPrefab, bet_prefabParent);
                    var Script = bet.transform.GetComponent<SuperRouletteBetHistoryDisplay>();
    ...
    [System.Serializable]
    public class SPBetResponse
    {
        public bool success { get; set; }
        public List<SPBetDatum> betHistory { get; set; }
    }
}
