...
public class JMIsWinnerManager : MonoBehaviour
{
    ...
    [Header("ApiEndPoint")]
    private string thunderballIsWinner_api_url = "http://13.234.117.221:2556/api/v1/user/iswinner_jhandimunda";
    ...
    IEnumerator IsWinner()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(thunderballIsWinner_api_url))
        {
            ...
            JMIsWinnerRoot isWinner = JsonConvert.DeserializeObject<JMIsWinnerRoot>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                if (isWinner.success == true)
                {
                    ...
                    var Script = victory_clone.transform.GetComponent<JMIsWinnerDisplay>();
                    if (Script != null)
                    {
...
[System.Serializable]
public class JMIsWinnerRoot
{
    public bool success { get; set; }
    public string message { get; set; }
    public JMIsWinnerData data { get; set; }
}

[System.Serializable]
public class JMIsWinnerData
{
