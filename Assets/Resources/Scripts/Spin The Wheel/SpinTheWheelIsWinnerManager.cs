...
public class SpinTheWheelIsWinnerManager : MonoBehaviour
{
    ...
    [Header("ApiEndPoint")]
    private string thunderballIsWinner_api_url = "http://13.234.117.221:2556/api/v1/user/iswinner_spinwheel";
    ...
    IEnumerator IsWinner()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(thunderballIsWinner_api_url))
        {
            ...
            STWIsWinnerRoot isWinner = JsonConvert.DeserializeObject<STWIsWinnerRoot>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                if (isWinner.success == true)
                {
                    ...
                    var Script = victory_clone.transform.GetComponent<SpinTheWheelIsWinnerDisplay>();
                    if (Script != null)
                    {
...
[System.Serializable]
public class STWIsWinnerRoot
{
    public bool success { get; set; }
    public string message { get; set; }
    public STWIsWinnerData data { get; set; }
}

[System.Serializable]
public class STWIsWinnerData
{
