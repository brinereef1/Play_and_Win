...
public class SpinTheWheelBetHistoryManager : MonoBehaviour
{
    ...
    private string bet_history_api_url = "http://13.234.117.221:2556/api/v1/user/userbethistory_spinwheel";
    ...
    IEnumerator BetHistoryRequest()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(bet_history_api_url))
        {
            ...
            STWBetResponse betResponse = JsonConvert.DeserializeObject<STWBetResponse>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                foreach (var item in betResponse.betHistory)
                {
                    GameObject bet = Instantiate(betPrefab, bet_prefabParent);
                    var Script = bet.transform.GetComponent<SpinTheWheelBetHistoryDisplay>();
...
[System.Serializable]
public class STWBetResponse
{
    public bool success { get; set; }
    public List<STWBetDatum> betHistory { get; set; }
}