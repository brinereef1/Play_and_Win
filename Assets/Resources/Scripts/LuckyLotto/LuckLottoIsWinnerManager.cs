...
public class LuckLottoIsWinnerManager : MonoBehaviour
{
[Header("WinPrefab Parent")]
    ...
    [Header("ApiEndPoint")]
    private string thunderballIsWinner_api_url = "http://13.234.117.221:2556/api/v1/user/isWinner_lucky_loto";
    ...
    IEnumerator IsWinner()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(thunderballIsWinner_api_url))
        {
            ...
            LLIsWinnerRoot isWinner = JsonConvert.DeserializeObject<LLIsWinnerRoot>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                if (isWinner.success == true)
                {
                    ...
                    var Script = victory_clone.transform.GetComponent<LuckyLottoIsWinnerDisplay>();
                    if (Script != null)
                    {
...
[System.Serializable]
public class LLIsWinnerRoot
{
    public bool success { get; set; }
    public string message { get; set; }
    public LLIsWinnerData data { get; set; }
}

[System.Serializable]
public class LLIsWinnerData
{
