...
public class PowerBallLastTenWinManager : MonoBehaviour
{
    ...
    private string api_url = "http://13.234.117.221:2556/api/v1/user/last10power";
    ...
    IEnumerator LastTenWinHistoryRequest()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(api_url))
        {
            ...
            PBLastTenWinResponse lastTen_winResponse = JsonConvert.DeserializeObject<PBLastTenWinResponse>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                foreach (var item in lastTen_winResponse.games)
                {
                    ...
                    var Script = win.transform.GetComponent<PowerBallLastTenWinDisplay>();
...
[System.Serializable]
public class PBLastTenWinResponse
{
    public List<PBLastTenDatum> games { get; set; }
}