...
public class PattiWinHistoryManager : MonoBehaviour
{
    ...
    private string win_history_api_url = "http://13.234.117.221:2556/api/v1/user/wiininghistory_fatafat";
    ...
    IEnumerator WinHistoryRequest()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(win_history_api_url))
        {
            ...
            PattiWinResponse PattiWinResponse = JsonConvert.DeserializeObject<PattiWinResponse>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                foreach (var item in PattiWinResponse.data)
                {
                    GameObject win = Instantiate(winPrefab, win_prefabParent);
                    var Script = win.transform.GetComponent<PattiWinHistoryDisplay>();
...
[System.Serializable]
public class PattiWinResponse
{
    public bool success { get; set; }
    public List<PattiBallDatum> data { get; set; }
}