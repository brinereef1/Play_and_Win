...
public class PattiBetHistoryManager : MonoBehaviour
{
    ...
    private string bet_history_api_url = "http://13.234.117.221:2556/api/v1/user/userbethistory_fatfafat";
    ...
    IEnumerator BetHistoryRequest()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(bet_history_api_url))
        {
            ...
            PattiBetResponse betResponse = JsonConvert.DeserializeObject<PattiBetResponse>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                foreach (var item in betResponse.betHistory)
                {
                    GameObject bet = Instantiate(betPrefab, bet_prefabParent);
                    var Script = bet.transform.GetComponent<PattiBetHistoryDisplay>();                   
 ...
 [System.Serializable]
    ...
    [System.Serializable]
    public class PattiBetResponse
    {
        public bool success { get; set; }
        public List<PattiBetDatum> betHistory { get; set; }
    }