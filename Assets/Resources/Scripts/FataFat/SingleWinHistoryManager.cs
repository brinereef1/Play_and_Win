...
public class SingleWinHistoryManager : MonoBehaviour
{
    ...
    private string win_history_api_url = "http://13.234.117.221:2556/api/v1/user/wiininghistory_fatafatsingle";
    ...
    IEnumerator WinHistoryRequest()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(win_history_api_url))
        {
            ...
            SingleWinResponse SingleWinResponse = JsonConvert.DeserializeObject<SingleWinResponse>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                foreach (var item in SingleWinResponse.data)
                {
                    GameObject win = Instantiate(winPrefab, win_prefabParent);
                    var Script = win.transform.GetComponent<SingleWinHistoryDisplay>();
...
[System.Serializable]
public class SingleWinResponse
{
    public bool success { get; set; }
    public List<SingleBallDatum> data { get; set; }
}