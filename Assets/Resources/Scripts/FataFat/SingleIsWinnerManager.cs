...
public class SingleIsWinnerManager : MonoBehaviour
{
    ...
    [Header("ApiEndPoint")]
    private string thunderballIsWinner_api_url = "http://13.234.117.221:2556/api/v1/user/iswinner_fatafatsingle";
    ...
    IEnumerator IsWinner()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(thunderballIsWinner_api_url))
        {
            ...
            Debug.Log("Response from SingleIsWinManager: " + response);
            SingleiIsWinnerRoot isWinner = JsonConvert.DeserializeObject<SingleiIsWinnerRoot>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                if (isWinner.success == true)
                {
                    ...
                    var Script = victory_clone.transform.GetComponent<PattiIsWinnerDisplay>();
                    if (Script != null)
                    {
...
[System.Serializable]
public class SingleiIsWinnerRoot
{
    public bool success { get; set; }
    public string message { get; set; }
    public SingleIsWinnerData data { get; set; }
}

[System.Serializable]
public class SingleIsWinnerData
{
