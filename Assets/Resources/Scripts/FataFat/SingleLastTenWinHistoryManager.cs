...
using System.Linq;
public class SingleLastTenWinHistoryManager : MonoBehaviour
{
    ...
    [Header("Numbers Parent")]
    public Transform Number_prefabParent;
    [Header("winPrefab")]
    public GameObject LastTenWin_prefab;
    [Header("AuthenticationToken")]
    public string AuthTok;

    private string api_url = "http://13.234.117.221:2556/api/v1/user/last10fatafatsingle";
    ...
    {
        cardList.Clear();
        foreach (Transform card in Number_prefabParent)
        {
    ...
    IEnumerator LastTenWinHistoryRequest()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(api_url))
        {
            ...
            SingleLastTenWinResponse lastTen_winResponse = JsonConvert.DeserializeObject<SingleLastTenWinResponse>(response);
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                foreach (var item in lastTen_winResponse.games)
                {
                    ...
                    var Script = win.transform.GetComponent<SingleLastTenDisplay>();
...
[System.Serializable]
public class SingleLastTenDatum
{
...
[System.Serializable]
public class SingleLastTenWinResponse
{
    public List<PSLastTenDatum> games { get; set; }
}